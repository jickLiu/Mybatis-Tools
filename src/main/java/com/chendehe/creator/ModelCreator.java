...
public class ModelCreator extends AbstractCreator {
    ...
    public void createModel() {
        table.getTableFields()
            .forEach(new BiConsumer<String, List<Field>>() {
                @Override
                public void accept(String longTableName, List<Field> fields) {
                    FileUtils.writeToFile(
                            FileUtils.createFile(table.getModelFolder().concat(ModelCreator.this.getModelName(longTableName).concat(JAVA))),
                            ModelCreator.this.createModelFileContent(longTableName));
                }
            });
